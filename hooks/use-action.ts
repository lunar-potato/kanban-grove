  ...
  const execute = useCallback(
    async (input: TInput) => {
      ...
      try {
        ...
        if (result.error) {
          // if there is a general error, updates the error state and calling onError callback
          setError(result.error);
          options.onError?.(result.error); // executes the onError callback
        }

        if (result.data) {
          // if successful and returns data, updates the data state and calling the onSuccess callback
          setData(result.data);
